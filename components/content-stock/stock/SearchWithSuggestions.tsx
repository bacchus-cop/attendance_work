...
export const SearchWithSuggestions: React.FC<SearchWithSuggestionsProps> = React.memo(({
    ...
    return (
        <div className="flex-1 flex flex-col" ref={searchContainerRef}>
            ...
            {/* Inline Quick Tags (Top 10) */}
            {top10Tags.length > 0 && (
                <div className="flex items-center gap-2 mt-2 px-1 text-xs select-none overflow-x-auto scrollbar-thin pb-1">
                    <span className="text-gray-400 font-bold whitespace-nowrap flex items-center gap-1 text-[11px]">
                        <TrendingUp className="w-3.5 h-3.5 text-indigo-500" /> แท็กยอดนิยม:
                    </span>
                        ...
                        {top10Tags.map((tag) => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => handleTagSuggestionClick(tag.name)}
                                    className={`
                                        inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-black transition-all duration-200 active:scale-95 whitespace-nowrap border
                                        ${isSelected 
